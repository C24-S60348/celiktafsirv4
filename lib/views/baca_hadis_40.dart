...
class _BacaHadis40PageState extends State<BacaHadis40Page> {
  ...
  /// Copies the loaded article body. The fetch fills [_hadis40Content] on load,
  /// so there is nothing to await here -- an empty field means the article has
  /// not landed yet (or failed), and we say so instead of copying nothing.
  void _copyContent() {
    if (_hadis40Content == null || _hadis40Content!.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
    ...
    _copyTextToClipboard(htmlToPlainText(_hadis40Content!), type: 'Kandungan');
  }
