...
class LaaTahzanPage extends StatefulWidget {
  const LaaTahzanPage({super.key});

  @override
  _LaaTahzanPageState createState() => _LaaTahzanPageState();
}

class _LaaTahzanPageState extends State<LaaTahzanPage> {
  ...
  void _loadPosts() async {
    ...
    final postList = await getlaatahz.GetLaaTahzan.getLaaTahzanPosts();

    setState(() {
      // Only keep the first 7 items (remove artikel 8 and 9)
      posts = postList.length > 7 ? postList.take(7).toList() : postList;
      isLoading = false;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('La Tahzan', style: TextStyle(color: Colors.white)),
        centerTitle: true,
