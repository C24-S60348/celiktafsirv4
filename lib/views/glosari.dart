...
class _GlosariPageState extends State<GlosariPage> {
  ...
  /// Copies the loaded glosari body. The article is fetched into
  /// [_glosariContent] on load, so there is nothing to await here -- if it is
  /// still empty the fetch has not landed (or failed) and we say so.
  void _copyContent() {
    if (_glosariContent == null || _glosariContent!.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
    ...
    _copyTextToClipboard(htmlToPlainText(_glosariContent!), type: 'Kandungan');
  }
                ...
                slivers: [
                  SliverAppBar(
                    ...
                    actions: [
                      ...
                      IconButton(
                        onPressed: () => showOpenWebsiteOverlay(
                          context,
