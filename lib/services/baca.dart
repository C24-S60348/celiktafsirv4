...
class BacaService {
  ...
  static Future<String?> fetchContentFromUrl(String url, String className) async {
    try {
  ...
  static Future<Map<String, dynamic>> getSurahWithTafsir(Map<String, String> surahData) async {
    String? tafsirContent = await fetchTafsirContent(surahData['name']?.toLowerCase() ?? '');
    
    return {
      'surah': surahData,
      'tafsir': tafsirContent,
      'tafsirText': tafsirContent != null ? parseHtmlToText(tafsirContent) : null,
    };
  }
}
